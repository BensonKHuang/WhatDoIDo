...
import wdid.Recommendations.Food;
...
public class ClubFactory implements RecommendationFactory {
  ...
  public RecommendationIterator getDummyRecommendations() {
    ...
    for(int i = 1; i <= 5; i++)
      dummy.add(new Food("Club " + Integer.toString(i), new Location(0.0, 0.0), "Desc " + Integer.toString(i), (double) i));
  ...
  public List<Recommendation> getData(WDIDUser user) {
    ...
    try {
      String urlString =
          String.format(
              "https://maps.googleapis.com/maps/api/place/nearbysearch/json?location=%f,%f&radius=1500&type=night_club&key=AIzaSyCjJ9D9zTYhgy0ffR_7sNdHQuzue__ailA",
              latitude, longitude);
